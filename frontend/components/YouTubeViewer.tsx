  ...
  return (
    <div className='w-full max-w-4xl mx-auto p-6 space-y-6'>
      <div className='flex flex-col space-y-2'>
        <h2 className='text-2xl font-bold tracking-tight'>YouTube Channel Videos</h2>
      ...
      <div className='flex gap-2'>
        ...
        >
          {loading && !videos.length ? <Loader2 className='w-4 h-4 animate-spin' /> : <Search className='w-4 h-4' />}
          Fetch
        </button>
      </div>
      {error && (
      ...
      <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
        {videos.map((video) => (
          <Link
            ...
            href={`/summarize?videoUrl=${encodeURIComponent(`https://www.youtube.com/watch?v=${video.id.videoId}`)}`}
          ...
          >
            ...
            <div className='space-y-1'>
              ...
              <p className='text-xs text-gray-500 dark:text-gray-400'>
                {new Date(video.snippet.publishedAt).toLocaleDateString(undefined, {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                })}
